...
## 🔴 PRIORITAS 28 — BUG: MENU DEMO MUNCUL DI POS WALAU CLOUD SYNC "CONNECTED" & SUDAH DI-REFRESH (Analisa, v4.10)

> **Sumber**: `ANALYSE.md` bagian Q (Q.1–Q.6) — audit lengkap + langkah verifikasi lapangan + audit ulang side effect.
...
- **Aksi (saat eksekusi)**:
  - [ ] Push hanya bila katalog lokal BUKAN seed murni — penanda `catalogTouched`/`inventoryTouched` (diset saat user tambah/edit/hapus/import menu/inventori; tidak diset untuk baseline seed) — opsi Q.4-1b; alternatif: dialog konfirmasi "Cloud kosong — unggah katalog lokal (N menu)?" (Q.4-1c)
  - [ ] Terapkan konsisten: `menuStore.loadFromCloud` (menus ±213), `inventoryStore.loadFromCloud` (items ±373-385), dan `menuStore.ts:225` (`customCategories` kosong → jangan push seed kategori)
  - [ ] Putuskan nasib `isFactoryResetSeedSkip`/`clearFactoryResetSeedSkip` (`factoryResetFlag.ts` + `dataManager.ts:238`) — menjadi dead code bila cabang push diganti; hapus atau dokumentasikan sebagai no-op
...
- **Aksi (saat eksekusi)**:
  - [ ] Ubah return-value `loadFromCloud` (atau tambah state) agar caller tahu fetch gagal vs kosong
  - [ ] Toast/banner "Katalog gagal dimuat dari cloud — menampilkan data lokal" saat error asli (bukan saat not-configured)
...
- **Aksi (saat eksekusi)**:
  - [ ] `deletedMenuIds: string[]` (cap) di menuStore + persist bersama `rempah-menus`
  - [ ] `deleteMenu` menulis tombstone; exclude saat merge dari `cloudMenus` DAN `localOnly`
