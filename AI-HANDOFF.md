...
| `TO DO.md` | Daftar lengkap temuan audit + status pengerjaan (**Prioritas 1–17 semuanya ✅ — termasuk Promo P-A2–P-A8, Mode Offline O-1–O-10, Printer Thermal 14.1–14.6, UX Kasir 15.1–15.4, Bug Item Pending 16.1, & UX/Anti-Duplikat 17.1–17.3** — ringkasan v4.5 di §10, v4.6 di §11, v4.7 di §12–§22) — wajib dibaca |
...
## 22. Riwayat Pengerjaan v4.7 — Prioritas 17: UX Edit Menu, Checkbox Cetak & Duplikat Transaksi Pending (17.1–17.3 ✅)

> **Temuan user (3 case)**: (1) toggle Best Seller/Level Gula/Pilihan Suhu di Edit Menu menumpuk; (2) checkbox "Cetak struk kasir" & "Cetak tiket dapur" ingin **berdampingan** di desktop; (3) pending yang diedit (tambah/kurangi) lalu dibayar → riwayat transaksi jadi **2**: 1 pending lama (masih Pending) + 1 transaksi Selesai (hasil edit).

### 22.1 Diagnosis (root cause)

- **17.1 (UI)**: baris checkbox Best Seller ⭐ / Level Gula 🍬 / Pilihan Suhu 🌡️ di `Catalog.tsx` adalah **anak langsung grid `grid-cols-1 sm:grid-cols-2`** → di desktop hanya mengisi **satu kolom grid (½ lebar modal)** → label panjang saling mendesak/wrap tidak rapi ("menumpuk").
- **17.2 (UI)**: dua checkbox cetak di `POS.tsx` (dan `SplitBillModal.tsx` dari perluasan 15.3) tersusun **vertikal** (`flex flex-col gap-1`) → memakan tinggi modal pembayaran.
- **17.3 (BUG KRITIS)**: identitas pending (`currentPendingTx` + `checkoutTxId`) adalah **component state POS** → hilang saat POS di-unmount (pindah halaman/refresh), sementara **cartStore PERSIST** (IndexedDB) → item hasil resume tetap ada → `finalizeTransaction` memakai `checkoutTxId` yang sudah **UUID BARU** → engine membuat **transaksi baru**, pending lama tidak pernah di-update → **2 transaksi** (pending lama masih Pending + Selesai baru). Engine & store sendiri sudah benar (re-commit ID sama → upsert by ID → 1 transaksi); celahnya hanya **ID yang dipakai finalize ≠ ID pending setelah remount**. (Bug laten se-area: blok sukses finalize tidak memanggil `setCurrentPendingTx(null)` → identity bocor ke order berikutnya → order BARU ikut `overrideQueueNumber`/`reservedDeductions` lama → stok terpotong salah.)

### 22.2 Perbaikan (17.1–17.3)

- **17.1** (`Catalog.tsx`): baris checkbox → `flex flex-wrap items-center gap-x-5 gap-y-2 h-full pt-6 sm:col-span-2` — **`sm:col-span-2`** membentang penuh di grid 2 kolom (desktop), **`flex-wrap` + `gap-y-2`** wrap rapi di layar sempit/mobile.
- **17.2** (`POS.tsx`, `SplitBillModal.tsx`): container dua checkbox → `flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-6` — **vertikal di mobile, berdampingan di desktop**; catatan "(tidak ada cetakan sama sekali)" diberi `sm:basis-full` agar wrap ke baris sendiri saat mode row.
- **17.3**:
  - `src/store/cartStore.ts` — field persist **`resumeContext?: { id, queueNumber?, kitchenStatus? } | null`** + `setResumeContext(ctx)`; **bersih otomatis** di `clearCart()` (abandon / sukses save-finalize / onCompleteSplit) & saat `removeItem` membuat keranjang **kosong** (semua item dihapus = resume dibatalkan).
  - `src/utils/pendingResume.ts` (baru) — helper murni **`resolveResumeRestore(ctx, cartItems, transactions)`**: tanpa konteks → jangan restore; tx tidak ditemukan / sudah **Selesai** (dibayar/dibatalkan di device lain) → **STALE** (bersihkan konteks agar tidak dipakai ulang); keranjang kosong → jangan restore; tx masih **Pending** + keranjang berisi → restore.
  - `src/pages/POS.tsx` — `handleResumePendingOrder` menyimpan `resumeContext`; **efek mount** memanggil `resolveResumeRestore` → `setCurrentPendingTx(tx)` + `setCheckoutTxId(tx.id)` sehingga `finalizeTransaction`/`handleSavePending` memakai **ID pending yang sama** (1 transaksi) dan semua logika turunan (`pendingItemsChanged`, `parentTx` split, status dapur, default skip tiket dapur) konsisten; konteks basi dibersihkan. **Bonus (bug laten)**: blok sukses finalize kini `setCurrentPendingTx(null)` — identity pending tidak bocor ke order berikutnya.
- **Test**: `src/test/pendingResumeContext.test.ts` (11 kasus) — (1) **reproduksi bug**: save pending → resume → state hilang (UUID baru) → finalize → **2 transaksi** ❌; (2) **dengan fix**: restore ID → finalize → **1 transaksi** (pending di-update ke Selesai) ✅; (3) fix + item ditambah → 1 transaksi berisi item hasil edit; (4) `resolveResumeRestore` 5 aturan (tanpa konteks / Pending+cart / Selesai→stale / tidak ditemukan→stale / cart kosong); (5) siklus hidup `resumeContext` di cartStore (set & clearCart / hapus item terakhir → bersih / hapus sebagian → dipertahankan).

### 22.3 Validasi & Status

- `npx tsc --noEmit` → **0 error**; `npx vitest run` → **460/460 test lolos** (44 file) — rantai terkini di §9.6.
- `npm run build` → **sukses (exit 0)** diverifikasi setelah seluruh Prioritas 17 (17.1–17.3 + sinkronisasi dokumen): built in **15.31s** (tsc + vite build + PWA generateSW), **51 entry precache (3606.39 KiB)** — konsisten (51 entries, ukuran naik tipis dari perubahan 17.1–17.3). Satu-satunya warning adalah chunk > 500 kB (dikenal, bukan regresi). Tidak ada error baru — perubahan layout (`Catalog.tsx`, `POS.tsx`, `SplitBillModal.tsx`) & resumeContext (`cartStore.ts`, `pendingResume.ts`) lolos build produksi.
- **TO DO.md**: Prioritas 17 **TUNTAS (17.1 + 17.2 + 17.3)** — Prioritas 1–17 selesai semua.
- **Dokumen rilis tersinkron (Prioritas 17)**: CHANGELOG.md (blok fitur baru + bullet perbaikan bug + 460/460), RELEASE-v4.7.md (Fitur Baru #11 + Perbaikan Utama + 460/460). Perubahan kode Prioritas 17 (5 file modified: `Catalog.tsx`, `POS.tsx`, `SplitBillModal.tsx`, `cartStore.ts`, `TO DO.md` + 2 file baru: `src/utils/pendingResume.ts`, `src/test/pendingResumeContext.test.ts`) masih **belum di-commit** (menunggu instruksi).

---

## 23. Riwayat Pengerjaan v4.7 — Prioritas 18: Skenario 2 Kasir & Offline (TUNTAS)

> Sesi lanjutan dari §22 (Prioritas 17). Seluruh **Prioritas 18 (18.1–18.8)** di `TO DO.md` selesai: proteksi stok & nomor antrean lintas kasir (RPC atomik), satu shift aktif, expected cash tersinkron, banner laporan belum final, fitur Catat Demo, promo anti-race, dan seluruh temuan A (A1–A13) + E7 dari ANALYSE.md. Status dokumen: **TO DO.md 18.1–18.8 ✅** — angka test **588/588** (56 file).

### 23.1 18.1 — RPC atomik stok (lost-update 2 kasir)

- **`supabase/schema.sql`**: fungsi **`adjust_inventory_stock(p_id, p_delta)`** — penyesuaian berbasis **DELTA** (`stock = stock + p_delta`) dengan guard `stock >= -p_delta` untuk deduksi; mengembalikan JSONB `{ok, stock, reason}` (`ok` / `insufficient` **dengan stok aktual cloud** / `not_found`). **Migration 27** di `runMigrations` (probe PGRST202 + cetak SQL untuk DB lama + flag `inventoryStockRpc`).
- **`cloudSync.ts`**: **`adjustInventoryStockCloud(adjustments, items)`** — online + RPC ada → panggil RPC per id (ditolak → `conflicts`); offline/RPC belum dibuat → **fallback absolut** (perilaku lama, di-queue) dengan penanda `degraded` — tidak ada transaksi yang gagal.
- **`inventoryStore.ts`**: `deductStock` (delta negatif) & `revertStock` (delta positif) memakai jalur atomik; **handler konflik oversell**: stok lokal **dikoreksi ke nilai cloud** + **stock log tipe 'adjust'** + **toast** "kemungkinan sudah terjual perangkat lain — periksa stok fisik".
- **Test**: `stockAtomicDeduct.test.ts` (7: 2 kasir, revert positif, fallback PGRST202/offline/tanpa konfigurasi, not_found, campuran) + `stockDeductConflict.test.ts` (5: koreksi + log + toast, multi-konflik, tanpa konflik). Mock cloudSync di 7 test file diperbarui; 2 assertion `syncInventoryStock` → `adjustInventoryStockCloud`.

### 23.2 18.2 — Nomor antrean atomik lintas kasir

- **`supabase/schema.sql`**: tabel **`queue_counters (outlet_id, date, last_number)`** + RLS/policy + fungsi **`allocate_queue_number(p_date, p_outlet, p_min)`** (row-lock upsert — dua kasir online mustahil mendapat nomor sama; `p_min` = floor `max(cloudMax, localMax)` anti-tabrakan data lama; INSERT pertama = `p_min + 1`). **Migration 28** (probe + SQL + flag `queueCounterRpc`).
- **`cloudSync.ts`**: `fetchMaxQueueNumberCloud(date)` + `allocateQueueNumberCloud(date, floor)`; **`transactionStore.getNextQueueNumber`** ditulis ulang: floor → RPC → **fallback `floor + 1`** (offline / RPC belum dibuat) — perilaku lama tetap, tanpa transaksi gagal.
- **Badge "#N duplikat"** (fallback): helper murni **`findDuplicateQueueNumbers`** (`src/utils/queueNumber.ts`, Demo/Cancel dikecualikan, reset harian dihormati) di Riwayat Transaksi & card Pending Payments — tooltip arahan void salah satu. **Renumber sengaja TIDAK dibuat** (struk tercetak tidak bisa diubah) — pencegahan (RPC) + visibilitas (badge).
- **Test**: `queueNumber.test.ts` (max & deteksi duplikat + kontrak RPC 4 simulasi) + `queueNumberAllocation.test.ts` (6 alur `getNextQueueNumber`).

### 23.3 18.3 — Satu shift aktif per outlet + expected cash tersinkron

- **`src/utils/shiftStats.ts`** (baru): helper murni **`computeShiftStats`** — SEMUA kasir dalam window shift, transaksi Selesai non-split + Kas Masuk/Keluar (shiftId match, fallback window).
- **`shiftStore.ts`**: `openShift` async + **guard 1 shift/outlet** (lokal + cloud; offline tetap diizinkan 1 per device), **`resumeExistingShift()`** (lanjutkan shift kasir lain tanpa input modal kas), `loadFromCloud` **restore shift terbuka PALING AWAL** (semua device menyatu; duplikat legacy tidak di-auto-close — hanya di-warn).
- **`OpenShiftModal.tsx`**: deteksi shift terbuka → banner "Lanjutkan Shift Ini" (nama kasir, jam, modal) + audit log `resume_shift`.
- **`Layout.tsx`**: modal tutup shift → **flush antrean → fetch ulang transaksi/shift/cash movements → muat ulang store** → expected cash dari data tersinkron (kegagalan sync tidak menggagalkan tutup shift — pola 6.4) + **banner kuning/merah "N data belum tersinkron (+M gagal)"** + tombol "Kirim & Muat Ulang"; ringkasan shift window-based.
- **🐛 Bonus fix (bug nyata 18.2)**: `t.date.startsWith(todayStr)` membandingkan tanggal **UTC** vs **lokal** → transaksi jam **00:00–07:00 WIB** terlewat dari floor & deteksi duplikat. Fix: **`toLocalDateKey()`** di `localMaxQueueNumber`/`findDuplicateQueueNumbers`, `addTransaction`, normalisasi `loadFromCloud`, dan range query `fetchMaxQueueNumberCloud`. +4 test timezone-agnostic.
- **Test**: `shiftStats.test.ts` (6) + `shiftStore.test.ts` (9: guard lokal/cloud/offline, resume, restore earliest, tutup lintas device).

### 23.4 18.4 — Batasan reserve split per-device terdokumentasi + warning UI

- **`splitStockSession.ts`**: header dokumentasi eksplisit — sesi reserve di localStorage device pembuat (`rempah-split-stock-session`); device lain tidak tahu reserve → bisa oversell (kelas E2); split FRESH = sesi satu device (tidak bisa di-resume dari device lain); jangka panjang: simpan reserve sebagai transaksi Pending/split-reserve di cloud (tercatat, belum dikerjakan).
- **`SplitBillModal.tsx`**: banner amber "**Stok item di-reserve hanya di device ini** — kasir lain di device berbeda tidak mengetahui reserve stok ini…" via helper murni **`isFreshSplitReserveActive(parentTx, session)`** (hanya split FRESH; split PENDING tidak — stoknya sudah dipotong saat pending dibuat & terlihat lintas device). +3 test.

### 23.5 Temuan A4 + A6 — race double-refund & signature cart harga add-on

- **A4**: `canExecuteRefund(tx, storeTransactions, hasSplitChildren, inFlight)` di `refund.ts` — cek ulang `refunded` dari **STORE** (bukan salinan render) + guard in-flight + split guard; `executeRefund` memakainya + `refundingRef`; tombol Konfirmasi ber-state "Memproses…" (disabled), Batal disabled saat processing, Refund per-row disabled saat PIN pending. +6 test.
- **A6**: `computeCartSignature` kini menyertakan **harga add-on** (`a.name:a.price`); **`cartSignatureMatches(stored, items)`** mencocokkan format baru ATAU **legacy** (sesi tersimpan pra-upgrade tidak boleh membuat reserve tidak ter-release → double deduction) — dipakai di `releaseSplitReserveForCart`, efek deteksi cart berubah & rehydrate paidState. +5 test.

### 23.6 Temuan A1–A3 — dead param, deduksi per-item, rollback berjejak

- **A1**: `skipStockDeduction` DIHAPUS dari `AtomicCheckoutParams` + cabang kosong di engine (tidak ada caller; jalur delta `reservedDeductions` satu-satunya cara melewati deduksi penuh).
- **A2**: `calculateItemDeductions` **per item** — item dengan snapshot memakai snapshot (`continue`), item tanpa snapshot fallback legacy (sebelumnya SATU flag global membuat campuran kehilangan deduksi legacy). +6 test (`hpp.test.ts`).
- **A3**: `executeRollback` menghitung delta dari snapshot lalu `revertStock(delta, 'Rollback transaksi gagal (stok dikembalikan)')` — log **'add'** balasan + sync bulk (sebelumnya `updateItem(skipLog)` → jejak audit tidak seimbang). +2 test (`engineRollback.test.ts`).

### 23.7 Temuan A5 — last-write-wins stok via `updatedAt` (Migration 29)

- Kolom `updated_at` sudah ada di CREATE TABLE inventory; **Migration 29** (ALTER idempoten + flag `inventoryUpdatedAt`). `InventoryItem.updatedAt` + helper murni **`isLocalNewer(local, cloud)`** (`src/utils/inventoryFreshness.ts`); SEMUA mutasi stok stamp `updatedAt`; payload sync kirim `updated_at`; `fetchInventoryFromCloud` baca `updated_at`; `loadFromCloud` **LWW per item** (lokal lebih baru dipertahankan, versi cloud yang kalah tidak di-merge anti duplikat; legacy → cloud otoritatif). +15 test. **Sisa risiko**: fallback absolut (offline/DB lama) tetap LWW-by-time — SOP cek stok pagi disarankan (RPC atomik 18.1 menutup online).

### 23.8 Keputusan desain A7/A8/A9 — nomor split, rekonsiliasi pending, SOP void

- **A7**: **1 pesanan = 1 nomor** — `resolveSplitQueueNumber(parentTx, session)` (split pending → nomor PARENT; fresh → nomor sesi); `findDuplicateQueueNumbers` mengecualikan sub-bill split (fix false-positive latent split fresh). +5 test.
- **A8**: cart tidak dikunci — **rekonsiliasi reserve SEKALI per parent** saat sub-bill pertama dibayar: `computePendingSplitReconcile` (item dihapus → revert, ditambah → deduct) + toast info; idempoten & tanpa double-adjust. +5 test.
- **A9**: **SOP void satu per satu** — toast peringatan jelas saat void transaksi ber-anak split (alur sekali-klik revert gabungan sengaja TIDAK dibuat — risiko double-revert & fabrikasi data; batasan desain terdokumentasi).

### 23.9 Temuan A10 — tiket dapur tidak hilang saat resume pending

- **`kitchenTicketPrintedAt`** di Transaction + kolom `kitchen_ticket_printed_at` (schema.sql + ALTER idempoten + **Migration 30** + flag `kitchenTicketPrintedAt`) + mapping `syncTransaction`/`syncTransactionMeta`.
- **Engine** (`triggerPostCommitTasks`): stamp HANYA bila tiket dapur benar-benar sukses (`didKitchenPrintSucceed`); printer gagal → tidak di-stamp.
- **Helper murni `shouldSkipKitchenPrintAtResume`** (`src/utils/kitchenTicket.ts`) dipakai di default modal checkout (`handleCheckoutCb` + `proceedCheckoutAnyway`): item berubah → cetak ulang; item sama & sudah cetak → skip (anti dobel); item sama & **belum pernah cetak** → **cetak ulang** (tiket tidak hilang). +9 test (`kitchenTicketPrint.test.ts`).

### 23.10 Temuan A11–A13 — warning bahan hilang, alert presisi, jalur Demo

- **A11**: `InventoryEngine.validateStockAvailability` melaporkan bahan resep yang sudah dihapus sebagai warning `missing: true` (checkout terblokir sementara, bisa "Lanjutkan Tetap") + pesan khusus di modal peringatan stok POS. +2 test.
- **A12**: `revertStock` hanya membersihkan `lastNegativeStockAlerts` bila revert benar-benar mengeluarkan item dari negatif; item masih negatif dipertahankan (stok terbaru). +3 test.
- **A13**: tombol **"Catat sebagai Demo (tidak memotong stok)"** di modal checkout POS → engine `overrideTxStatus: 'Demo'` **tanpa potong stok**, **queueNumber 0** (tidak konsumsi nomor antrean RPC), `suppressAutoPrint`, tanpa kunjungan/promo/loyalty; Riwayat menampilkan **DEMO** (bukan #0) + pencarian "demo"; konversi Demo→Selesai tetap deduct stok (8.1); tombol disembunyikan saat resume pending (stok pending sudah terpotong). +3 test (`demoTransaction.test.ts`).

### 23.11 Temuan E7 — promo usage & loyalty anti-race

- **`reservePromoUsage(id, customerId, usageKey)`** di promoStore — cek batas (global & per pelanggan) dari **STORE saat commit** (bukan render) + increment **atomik** (satu functional set, re-check di dalam updater) + **ledger `usageKeys`** id unik transaksi → replay/re-commit transaksi sama mengembalikan `{ok, idempotent}` tanpa increment (sebelumnya `incrementUsage` jalan ulang saat idempotentReplay).
- **Replay guard efek samping** di POS finalize: `recordVisit`/`reservePromoUsage`/`deductLoyaltyPoints` hanya jalan bila `!result.idempotentReplay` — fix kunjungan ganda & poin loyalty terpotong 2× saat double-click.
- **Merge ledger UNION lintas device** di `loadFromCloud` (key monotonik — key yang tercatat di mana pun adalah pemakaian nyata). POS & SplitBillModal pakai `reservePromoUsage(subTx.id)` + toast bila batas tercapai (transaksi tetap diproses). **Residual terdokumentasi**: dua device OFFLINE hampir bersamaan tetap bisa lolos batas (sync promo LWW) — RPC counter opsional (pola 18.2). +8 test (`promoStoreUsage.test.ts`).

### 23.12 18.5 + 18.6 — review KDS aman + banner "Laporan belum final"

- **18.5**: review (tanpa fix wajib) — status dapur last-write-wins, tiket dapur per-printer device, offline queue kronologis (O-10).
- **18.6**: **`SyncFreshnessBanner`** (`src/components/SyncFreshnessBanner.tsx`) — banner amber "**Laporan belum final — N transaksi belum tersinkron ke cloud (+M operasi lain dalam antrean sinkron)**" di header **Laporan (semua tab)** & **Dashboard**. Sinyal: `unsyncedTx` dari mekanisme O-5 (`confirmedSyncIds`, reaktif via zustand) + jumlah operasi antrean (mount + interval 30 dtk + visibilitychange). **Keputusan desain**: TIDAK memakai `setQueueChangeListener` (satu slot listener global dipegang Layout/O-4 — akan saling menimpa). Logika murni `computeSyncFreshness` diuji unit (6 kasus).

### 23.13 Validasi & Status

- `npx tsc --noEmit` → **0 error**; `npx vitest run` → **588/588 test lolos** (56 file) — rantai terkini di §9.6.
- **TO DO.md**: Prioritas 18 **TUNTAS (18.1–18.8)** — termasuk seluruh temuan A (A1–A13) + E7 di blok 18.8; 18.7 ditandai superseded oleh E7.
- **SQL yang wajib dijalankan sekali di Supabase SQL Editor (DB lama)**: **Migration 27** (RPC `adjust_inventory_stock`), **Migration 28** (tabel `queue_counters` + RPC `allocate_queue_number`), **Migration 29** (`ALTER TABLE inventory ADD COLUMN IF NOT EXISTS updated_at`), **Migration 30** (`ALTER TABLE transactions ADD COLUMN IF NOT EXISTS kitchen_ticket_printed_at`) — semuanya idempoten, dicetak lengkap di console saat app start mendeteksi belum ada. Sampai dijalankan, aplikasi tetap berjalan dengan fallback aman (absolut / max+1) + badge deteksi.
- **Dokumen rilis tersinkron (Prioritas 18)**: CHANGELOG.md (blok "Skenario 2 Kasir & Offline" + bullet perbaikan + SQL butir 12–15 + 588/588), RELEASE-v4.7.md (Fitur Baru #12 + Perbaikan Utama + SQL butir 12–15 + 588/588), DEPLOYMENT.md (§4 butir 12–15 + Migration 27–30 di self-healing + checklist §7 + tabel §8 + 588/588).
- **`npm run build` ✅ BERHASIL** (18 Agt 2026, setelah seluruh Prioritas 18 tuntas): `✓ built in 16.61s`, PWA v1.3.0 `generateSW` → **50 precache entries (3622.00 KiB)**, `dist/sw.js` + `dist/workbox-c3716bd4.js` digenerate; hanya warning chunk > 500 kB (Reports 490.61 kB, index 648.86 kB — kosmetik, bukan error). Tidak ada error TypeScript/rollup. Build produksi **v4.7 final diverifikasi**.
- Perubahan kode sesi 18.1–18.6 + A1–A13 + E7 masih **belum di-commit** (menunggu instruksi).

## 24. Riwayat Pengerjaan v4.7 — Prioritas 20: Audit Fitur Eksisting (20.1 ✅ + 20.2 ✅ + 20.3 ✅ + 20.4 ✅)

> Audit menyeluruh fitur yang sudah ada pasca-Prioritas 18 (baseline 588/588 hijau): scan pola bug + penelusuran logika area berisiko (promo, refund, struk digital, shift, laporan). Analisa lengkap: `ANALYSE.md` Bagian G; daftar: `TO DO.md` Prioritas 20.

### 24.1 20.1 — Ringkasan tutup shift overstated saat refund (FIX)

- **Akar masalah**: `computeShiftStats` (`src/utils/shiftStats.ts`) menghitung `shiftTx` tanpa mengecualikan `t.refunded` → Total Penjualan & Total Transaksi ringkasan shift termasuk transaksi yang sudah di-refund — **inkonsisten** dengan Dashboard/Reports/Transactions yang sudah exclude `!refunded`.
- **Analisa netting (kunci)**: `expectedCash = opening + cashSales + cashIn − cashOut`; movement Refund ('out') ikut dihitung. Untuk refund tunai, sale refunded (+cashSales) & movement (−cashOut) saling meniadakan → net 0. Meng-exclude refunded dari cashSales TANPA menyesuaikan movement = double-subtract (expectedCash `opening − refund` — salah).
- **Fix**: `salesTx = shiftTx.filter(t => !t.refunded)` menjadi basis laporan (`totalSales`/`totalTx`/`cashSales`/`qrisSales`/`transferSales`); field baru **`refundedCashSales`** (sale tunai refunded dalam window) di-**add-back** ke formula expected cash — secara matematis setara formula lama, netting netral, tanpa regresi kasus lintas metode (residual terdokumentasi: fix penuh butuh field `refundMethod`). UI: baris **"↩️ Refund Tunai (Dikembalikan)"** di modal tutup shift + struk ringkasan bila > 0 (`Layout.tsx`).
- **Test (+5)** `shiftStats.test.ts`: refund tunai (totalSales bersih + expectedCash net 0), campuran sale aktif + refunded, refund QRIS dari laci (expectedCash − refund), lintas metode (tidak regresi), refunded tanpa movement (tidak double-count).

### 24.2 20.2 — Semua `alert()` diganti toast (FIX)

- **Akar masalah**: ~15 `alert()` tersisa di halaman non-printer (AuditLog, CashMovements, Catalog, SettingsPage, App) — inkonsisten dengan konvensi toast (Prioritas 14.6 baru menyentuh alur printer).
- **Fix**: **21 `alert()` → `useToastStore().addToast`** di 7 file: App.tsx (session takeover, via `getState()` di callback subscription), AuditLog.tsx (success/warning clear logs), CashMovements.tsx (warning nominal ≤ 0 ×2), Catalog.tsx (error foto / warning komponen), SettingsPage.tsx (10 titik: validasi pajak/meja/file/username/shift aktif + success simpan/tema/PIN Manager/PIN Super Admin), **StockOpname.tsx** (3 titik — temuan tambahan), **authStore.ts** (session takeover — temuan tambahan). `window.confirm` sengaja dipertahankan (konfirmasi destruktif, bukan target). Kode produksi **0 `alert()` tersisa**.
- **Pola**: komponen pakai hook `useToastStore()`; store/callback non-komponen pakai `useToastStore.getState().addToast(...)`.

### 24.3 20.3 — Semua `window.confirm` diganti ConfirmDialog (FIX)

- **Akar masalah**: 4 `window.confirm` tersisa (terlewat di 20.2 karena itu bukan `alert()`) — UX konfirmasi tidak seragam (dialog browser native vs ConfirmDialog kustom).
- **Fix**: 4 titik diganti **ConfirmDialog** (`src/components/ConfirmDialog.tsx`): (1) `Layout.tsx:826` tutup shift selisih kas > 10% — reuse pola `confirmState` yang sudah ada (O-10), dialog "Selisih Kas Besar" + tombol "Ya, Tutup Shift"; (2) `PendingPaymentsModal.tsx` void transaksi gantung — state `voidTarget` + dialog "Batalkan Transaksi Gantung" (peringatan stok dikembalikan); (3) `POS.tsx` resume pending saat keranjang berisi — dipecah `handleResumePendingOrder` → `applyResumePendingOrder`, state `resumeConfirmTx`, dialog variant warning; (4) `SettingsPage.tsx` hapus user — state `deleteUserTarget` + ConfirmDialog ke-4 (pesan + audit log `delete_user` dipindah ke `onConfirm`).
- **Hasil**: kode produksi **0 `window.confirm`, 0 `alert()`, 0 `window.prompt`** — seluruh dialog aplikasi memakai toast + ConfirmDialog/Modal kustom.

### 24.5 20.4 — Filter tanggal custom Laporan & Riwayat memakai UTC untuk tanggal awal (FIX)

- **Akar masalah**: `new Date(customDateFrom)` dengan format `"YYYY-MM-DD"` (tanpa `T`) di-parse sebagai **UTC tengah malam = 07:00 WIB**, sementara `new Date(customDateTo + 'T23:59:59')` di-parse **lokal** → transaksi **00:00–07:00 WIB pada tanggal awal tidak masuk** range custom. Kelas bug sama dengan fix 18.3 (pagi buta) yang sudah diterapkan di queueNumber/transactionStore/cloudSync — filter custom Laporan/Riwayat terlewat. Titik: `Reports.tsx` (`filteredTx` + `dateFrom` opname/movement) & `Transactions.tsx` (filter custom).
- **Fix**: helper murni **`buildCustomDateRange(fromStr, toStr)`** di `src/utils/format.ts` — `from` parse **lokal** `'T00:00:00'`, `to` lokal `'T23:59:59.999'`, fallback epoch/now; dipakai di 3 titik (Reports `filteredTx`, Reports `dateFrom`/`dateTo` opname/movement, Transactions custom).
- **Test (+5)** `dateRange.test.ts`: start = lokal midnight di zona waktu mana pun (jam 0/menit 0/tanggal sesuai input), **regresi utama transaksi 03:00 lokal tanggal awal MASUK range**, transaksi 23:59:59.500 hari akhir masuk, sebelum/sesudah ter-exclude, fallback kosong.
...
- **TO DO.md**: Prioritas 20 **TUNTAS (20.1–20.4)** + Prioritas 21 **TUNTAS (21.1–21.5)** + Prioritas 23 **TUNTAS (23.1–23.7)** + Fix 24.x **TUNTAS (24.1–24.5)**; ANALYSE.md Bagian G ✅ + Bagian H ✅ + Bagian M ✅. **Prioritas 19 (Multi Outlet) tetap BELUM dieksekusi** (analisa F.1–F.10 di ANALYSE.md, daftar 19.1–19.14 di TO DO.md).
...
## 25. Riwayat Pengerjaan v4.7 — Prioritas 21: Audit Flow Pending + Tambah Item + Split Bill (21.1 ✅ + 21.2 ✅ + 21.3 ✅ + 21.4 ✅ + 21.5 ✅)

### 25.1 21.1 — Tiket dapur hanya cetak delta items saat finalisasi pending yang diedit (FIX)

- **Akar masalah**: Saat finalisasi pending yang diedit (`pendingItemsChanged = true`), engine cetak tiket dapur untuk SEMUA item di cart (termasuk item lama yang sudah diproses/diantar dapur) → tiket dobel.
- **Fix**: Tambah field `deltaKitchenItems?: CartItem[]` di `AtomicCheckoutParams`; engine (`triggerPostCommitTasks`) pakai `deltaKitchenItems` jika ada untuk tiket dapur (bukan `receiptData` penuh); POS.tsx hitung `deltaKitchenItems = cart.items.filter(ci => !parentTx.items.some(pi => pi.lineId === ci.lineId))` saat finalisasi pending dengan `pendingItemsChanged = true`.
- **File**: `src/types/index.ts`, `src/lib/atomicTransactionEngine.ts`, `src/pages/POS.tsx`, `src/test/kitchenTicketPrint.test.ts` (+4 test).

### 25.2 21.2 — Default `skipSplitKitchen = true` saat split dari pending (FIX)

- **Akar masalah**: `skipSplitKitchen` default = false → tiket dapur dicetak lagi saat split dari pending yang sudah punya tiket (Simpan Pending) → dobel.
- **Fix**: `SplitBillModal.tsx` set `setSkipSplitKitchen(!!parentTx)` saat modal dibuka konteks baru — split dari pending otomatis skip tiket; split fresh tetap cetak.
- **File**: `src/components/SplitBillModal.tsx`.

### 25.3 21.3 — Guard rekonsiliasi ganda: pending + split (FIX)

- **Akar masalah**: Rekonsiliasi stok bisa jalan 2x (SplitBillModal line 370 + POS.tsx `reservedDeductions`) → double-adjust.
- **Fix**: `SplitBillModalProps` dapat `onReconcile?: () => void` — dipanggil setelah rekonsiliasi stok; `POS.tsx` track `pendingSplitReconciled` — jika true, `reservedDeductions = undefined` saat finalisasi pending; reset saat `onCompleteSplit`.
- **File**: `src/components/SplitBillModal.tsx`, `src/pages/POS.tsx`.

### 25.4 21.4 — Badge visual "✓ Diupdate" di Pending Payments (FIX)

- **Akar masalah**: Tidak ada indikator di PendingPaymentsModal bahwa pesanan sudah diedit (tambah/kurangi item).
- **Fix**: Badge **"✓ Diupdate"** (biru) muncul di kartu pending jika `updatedAt > date + 5 detik` — deteksi otomatis tanpa field tambahan.
- **File**: `src/components/PendingPaymentsModal.tsx`.

### 25.5 21.5 — KDS: badge "🔄 Diupdate" + timer restart untuk pesanan Done→Waiting (FIX)

- **Akar masalah**: Pesanan sudah Done di KDS → pelanggan tambah item → kitchenStatus reset ke Waiting → pesanan muncul lagi di kolom "Antrean Menunggu" tanpa badge/tanda UPDATE; `isOverdue` dihitung dari `date` (bukan waktu update) → langsung muncul overdue.
- **Fix**: `isUpdatedOrder()` deteksi via `updatedAt > date + 5 detik`; badge "🔄 Diupdate" + background biru di kartu; `isOverdue`/`getWaitingMinutes` pakai `updatedAt` untuk order update (timer restart); catatan "Pesanan diperbarui — periksa item baru" di bawah daftar item; label waktu "X mnt (sejak update)".
...
## 28. Riwayat Pengerjaan v4.8 — Audit Pasca-Rilis 23 & Fix 24.1–24.5 (semua ✅)

### 28.1 24.1 — syncTransactionMeta tidak sync items ke cloud (FIX KRITIS)

- **Akar masalah**: `updateItemKitchenStatus` memanggil `syncTransactionMeta(txId, { items })` tapi `syncTransactionMeta` tidak memproses field `items` (hanya paymentMethod, refunded, kitchenTicketPrintedAt) → kitchenItemStatus tidak tersync ke cloud → device lain tidak melihat perubahan status item.
- **Fix**: Hapus guard `!migrationNeeded.kitchenItemStatus` yang tidak perlu. Field `items` (JSONB) langsung di-sync ke cloud via `smartUpdate`. Hapus `as any` cast di `transactionStore.ts`.
- **File**: `src/lib/cloudSync.ts`, `src/store/transactionStore.ts`.

### 28.2 24.2 — Tombol "Selesai Semua" bisa menandai item 'new' sebagai done (FIX)

- **Akar masalah**: Filter `kitchenItemStatus !== 'done'` → item 'new' (belum diproses) juga ikut ditandai done.
- **Fix**: Ubah filter ke `kitchenItemStatus === 'processing'` — hanya item yang sedang diproses yang ditandai selesai.
- **File**: `src/pages/Kitchen.tsx`.

### 28.3 24.3 + 24.4 — Delta & merge kitchen status (FIX)

- **Akar masalah 24.3**: `calculateDeltaKitchenItems` mengembalikan semua item delta dengan status 'new' → item yang sedang diproses di-reset ke 'new'.
- **Akar masalah 24.4**: `mergeKitchenItemStatus` tidak mengecek quantity/specs berubah → qty tambahan tidak mendapat badge 'Baru'.
- **Fix**: `mergeKitchenItemStatus` mengecek quantity/specs/addons berubah → status 'new'. `calculateDeltaKitchenItems` mempertahankan logika lama (delta qty = selisih 'new', specs = seluruh item 'new').
- **File**: `src/utils/kitchenTicket.ts`.

### 28.4 24.5 — Filter KDS mixed status (FIX)

- **Akar masalah**: Filter lama menghitung `effectiveStatus` satu status dominan → transaksi dengan item 'new' + 'processing' hanya muncul di satu kolom.
- **Fix**: Filter orders tampilkan di kolom JIKA ada item dengan status kolom tersebut. Filter items tampilkan item sesuai kolom (new→Waiting, processing→Processing, all→Done).
- **File**: `src/pages/Kitchen.tsx`.

### 28.5 Validasi & Status (Fix 24.1–24.5)

- `npx tsc --noEmit` → **0 error**; `npx vitest run` → **632/632 test lolos** (61 file).
- `npm run build` → **✅ BERHASIL** (9.76s, 50 chunks, PWA v1.3.0).
- **TO DO.md**: 24.1 ✅ + 24.2 ✅ + 24.3 ✅ + 24.4 ✅ + 24.5 ✅ (semua selesai).
