...
- `npx tsc --noEmit` → **0 error**; `npx vitest run` → **598/598 test lolos** (57 file; +5 dari 593) — rantai terkini di §9.6.
- **TO DO.md**: Prioritas 20 **TUNTAS (20.1 + 20.2 + 20.3 + 20.4)**; ANALYSE.md Bagian G → G-1 ✅ G-2 ✅ G-3 ✅ (+ catatan 20.3/20.4). **Prioritas 19 (Multi Outlet) tetap BELUM dieksekusi** (analisa F.1–F.10 di ANALYSE.md, daftar 19.1–19.14 di TO DO.md).
- **CHANGELOG.md**: 4 bullet Perbaikan Bug (shift refund + alert→toast + confirm→ConfirmDialog + filter tanggal custom lokal) + validasi 598/598. **RELEASE-v4.7.md**: 4 bullet Perbaikan Utama + 598/598 (intro + validasi). **DEPLOYMENT.md**: §2 + §7 checklist Prioritas 20 (20.1 s.d. 20.4) + §8 tabel diperbarui ke 598/598. **TESTING-PRADEPLOY.md**: tahap F (uji Prioritas 20 — ringkasan shift akurat pasca-refund + tanpa dialog browser native) + tabel ringkasan.
